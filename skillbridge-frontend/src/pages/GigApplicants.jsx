  ...
  const fetchData = async () => {
    try {
      const [gigRes, applicantsRes] = await Promise.all([
        api.get(`/api/gigs/${id}`),
        api.get(`/api/applications/${id}/applicants`)
      ]);
      setGig(gigRes.data.gig);
      setApplicants(applicantsRes.data.applicants);
      setMatchSource(applicantsRes.data.source || '');
    } catch (error) {
          ...
          <div className="text-right">
            {matchSource && (
              <p className="text-xs uppercase tracking-wide text-emerald-700 mb-1">
                Ranked by {matchSource === 'fastapi' ? 'FastAPI matcher' : 'local fallback'}
              </p>
            )}
              ...
              return (
                <div key={app.id} className="bg-white rounded-xl border border-gray-200 p-6 hover:border-emerald-300 transition-all">
                  ...
                  {typeof app.match_score === 'number' && (
                    <div className="mb-4 rounded-lg bg-emerald-50 border border-emerald-100 px-4 py-3">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-sm font-medium text-emerald-800">Skill Match Score</p>
                        <span className="text-sm font-semibold text-emerald-900">{app.match_score}%</span>
                      </div>
                      <p className="text-xs text-gray-600">
                        Matched: {(app.matched_skills || []).join(', ') || 'None'}
                      </p>
                      <p className="text-xs text-gray-600">
                        Missing: {(app.missing_skills || []).join(', ') || 'None'}
                      </p>
                    </div>
                  )}
